...
pygame.display.set_caption("krutoi paint")
...
# Функция для рисования панели кнопок
def setsurf():
    surf.fill('black')
    buttons.fill('white')
    ...
    eraser_img = pygame.image.load("eraser.png")  # Загружаем изображение
    eraser_img = pygame.transform.scale(eraser_img, (36, 28))  # Масштабируем под кнопку
    buttons.blit(eraser_img, (56, 58))  # Рисуем картинку

    screen.blit(surf, (0, 0))
    screen.blit(buttons, (700, 0))

# функция для отрисовки ромб
def rhombus(screen, start, end, d, color):
    x1 = start[0]
    y1 = start[1]
    x2 = end[0]
    y2 = end[1]

    b = abs(y1-y2) #высота ромба
    if x1 <= x2:
        if y1 < y2:
            pygame.draw.polygon(screen, color, [[x1+(b/2),y1], [x1,y1+b/2], [x1+b/2,y1+b],[x1+b,y1+b/2]],d)
        else:
            pygame.draw.polygon(screen, color, [[x1+(b/2),y1], [x1,y1-b/2], [x1+b/2,y1-b],[x1+b,y1-b/2]],d)
...
def sqare(screen, start, end, d, color):
    x1 = start[0]
    y1 = start[1]
    x2 = end[0]
    y2 = end[1]
    ...
    if x1 <= x2:
        if y1 < y2:
            pygame.draw.polygon(screen, color , [[x1,y1],[x1,y1+a] ,[x1+a,y1+a],[x1+a,y1]],d)
        else:
            pygame.draw.polygon(screen, color , [[x1,y1],[x1,y1-a] ,[x1+a,y1-a],[x1+a,y1]],d)
    else:
        if y1 < y2:
            pygame.draw.polygon(screen, color , [[x1,y1],[x1-a,y1] ,[x1-a,y1+a],[x1,y1+a]],d)
        else:
            pygame.draw.polygon(screen, color , [[x1,y1],[x1,y1-a] ,[x1-a,y1-a],[x1-a,y1]],d)

# функция для отрисовки прямоугольным треугольником
def right_triangle(screen, start, end, d, color):
    x1 = start[0]
    y1 = start[1]
    x2 = end[0]
    y2 = end[1]
    ...
    if x1 <= x2:
        if y1 < y2:
            pygame.draw.polygon(screen, color, [[x1,y1], [x1,y1+b], [x1+b,y1+b]],d)
        else:
            pygame.draw.polygon(screen, color, [[x1,y1],[x1,y1-b],[x1+b,y1-b]],d)
    else:
        if y1 < y2:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_4:
                cur_color = 'white'  
        # при нажатии на область определенной кнопочки меняем фигуру кисти
        if event.type == pygame.MOUSEBUTTONDOWN:
            for k, v in commands.items():
                if v[0] <= pos[0]-700 <= v[0] + v[2] and v[1] <= pos[1] <= v[1] + v[3]:
                    d[k] = True #включаем выбранную фигуру 
                    for i, j in d.items():
                        if i != k:
                            d[i] = False
                    break
